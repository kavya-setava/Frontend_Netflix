...
const ReusableTable = ({ columns, data, userRole }) => {
        ...
        <div style={{ position: 'relative', minHeight: '100vh' }}>
            ...
            {data?.map((row, index) => (
              <tr key={row.id || index} style={
                Number(userRole) === 1 && row?.asap
                  ? { backgroundColor: "#000000" } // Bootstrap warning yellow
                  : {}
              }>
                {columns?.map((col, colIndex) => (
                  <td key={colIndex} className="text-center">
                    ...
                    <tbody>
                        {data?.map((row, index) => {
                            // console.log('Row check:', row.ticketKey, row.asap, userRole);
                            return (
                                <tr key={row.id || index} className={userRole === 1 && row.asap ? 'table-warning' : ''}>
                                    {columns?.map((col, colIndex) => (
                                        <td key={colIndex} className="text-center">
                                            {col.render ? col.render(row, index) : row[col.key]}
                                        </td>
