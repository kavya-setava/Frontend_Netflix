...
const Tickets = () => {
  ...
  render: (row) => {
    ...
    return (
      <Select
        options={typeOptions}
        value={row.type ? { label: row.type, value: row.type } : null}
        ...
        onChange={(selectedOption) => {
          row.type = selectedOption?.value || null; // store selection in row
          row.subType = null; // reset subtype when type changes
        }}
      />
    );
  },
},
{
  ...
  key: 'subDropdown',
  render: (row) => {
    // 🔹 Lookup map for dependent dropdown
    const subTypeOptionsMap = {
      Start: [
        { value: 'Start-Option1', label: 'Start - Option 1' },
        { value: 'Start-Option2', label: 'Start - Option 2' },
      ],
      Interim: [
        { value: 'Interim-Option1', label: 'Interim - Option 1' },
        { value: 'Interim-Option2', label: 'Interim - Option 2' },
      ],
      'Solution Provided': [
      ...
      'Need More Information': [
        { value: 'NeedInfo-1', label: 'Need Info - 1' },
      ],
      Closed: [
        { value: 'Closed-1', label: 'Closed - 1' },
        { value: 'Closed-2', label: 'Closed - 2' },
      ],
      'Sent to VAO': [
        { value: 'VAO-1', label: 'VAO - 1' },
      ],
    };

    const subOptions = row.type ? subTypeOptionsMap[row.type] || [] : [];

    return (
      <Select
        options={subOptions}
        value={row.subType ? { label: row.subType, value: row.subType } : null}
        isDisabled={!row.type} // disable until type is selected
        classNamePrefix="react-select"
        styles={{
          container: (base) => ({
            ...base,
        ...
        onChange={(selectedOption) => {
          row.subType = selectedOption?.value || null;
        }}
      />
    );
  },
      ...
      <Card>
        ...
        <div className="mb-6 grid grid-cols-1 gap-6 text-white sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            return (
              <div key={idx} className="from-cyan-500 to-cyan-400">
                <div
                  className={`card text-white p-3 w-100 bg-${item.color !== 'custom' ? item.color : ''}`}
                  style={{
                    textAlign: 'center',
                    backgroundColor: item.color === 'custom' ? '#564d4d' : undefined,
                    cursor: 'pointer',
        ...
        <div className="d-flex gap-3 mt-4 flex-wrap align-items-center" style={{ display: 'flex' }}>
          {Number(user?.role) === 0 && (
            <>
              ...
              <div className="form-group pe-3 flex" style={{ minWidth: 250 }}>
